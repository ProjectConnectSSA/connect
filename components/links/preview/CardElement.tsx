...
export default function CardElement({ element, styles, updateElement }: CardElementProps) {
    ...
    <>
      {element.thumbnailUrl && (
        <img
          ...
          }`} // Apply top radius, or full if no text
        />
      )}
  ...
  return (
    <div className={`relative group mb-3 w-full ${element.layout === "double" ? "md:w-[calc(50%-0.5rem)]" : "w-full"}`}>
      ...
      <button
        ...
        className="absolute top-1 right-1 p-1 text-gray-400 bg-white bg-opacity-70 rounded-full hover:text-gray-800 hover:bg-opacity-90 opacity-0 group-hover:opacity-100 transition-opacity z-10"
