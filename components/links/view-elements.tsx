...
export function CountdownElementView({ element, styles }: { element: BioElement; styles: StyleProps }) {
  ...
  return (
    <motion.div className="my-4 p-4 rounded border shadow bg-white text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
...
export function HeaderElementView({ element, styles }: { element: BioElement; styles: StyleProps }) {
  return (
    <div className="my-4 flex items-center justify-center px-4">
      <h2 className="text-xl font-semibold text-center w-full" style={{ color: styles.textColor }}>
...
export function CardElementView({ element, styles }: { element: BioElement; styles: StyleProps }) {
  ...
  const cardContent = (
    <>
      {element.thumbnailUrl && <img src={element.thumbnailUrl} alt={element.title || "Card thumbnail"} className={`w-full h-32 object-cover ${radiusClass.replace("rounded-", "rounded-t-")}`} />}
  ...
  return (
    <div className={`mb-3 w-full ${element.layout === "double" ? "md:w-[calc(50%-0.5rem)]" : "w-full"}`}>
      {element.url ? (
        <a href={element.url} target="_blank" rel="noopener noreferrer" className={`block overflow-hidden border shadow-md ${radiusClass}`} style={{ backgroundColor: cardBgColor }}>
          {cardContent}
        </a>
      ) : (
        <div className={`block overflow-hidden border shadow-md ${radiusClass}`} style={{ backgroundColor: cardBgColor }}>
          {cardContent}
        </div>
      )}
    </div>
...
export function ImageElementView({ element, styles }: { element: BioElement; styles: StyleProps }) {
  ...
  return displayUrl ? <img src={displayUrl} alt={element.title || "User uploaded image"} className={`w-full h-auto object-cover ${radiusClass} my-3`} /> : null;
}
...
export function LinkElementView({ element, styles }: { element: BioElement; styles: StyleProps }) {
  ...
  return (
    <a href={element.url || "#"} target="_blank" rel="noopener noreferrer" className={styles.buttonStyle === "filled" ? filledStyle : outlineStyle} style={styles.buttonStyle === "filled" ? { backgroundColor: styles.buttonColor, color: styles.buttonTextColor } : { borderColor: styles.buttonColor, color: styles.buttonColor }}>
      {element.title || "Link"}
    </a>
  );
}
  ...
  return (
    <div className="my-6 flex justify-center space-x-4">
      {element.socialLinks?.map((link, index) => (
        <a key={index} href={link.url || "#"} target="_blank" rel="noopener noreferrer" style={{ color: styles.textColor }} className="hover:opacity-75 transition-opacity" aria-label={link.platform}>
