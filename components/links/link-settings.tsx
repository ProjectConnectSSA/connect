  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        <label className="text-sm font-medium text-gray-700">Page Status</label>
        <label className="relative inline-flex items-center cursor-pointer">
          <input
            ...
            disabled={isSaving || isLoading}
          />
          <span className="w-11 h-6 bg-gray-200 rounded-full peer-checked:bg-blue-600 transition-colors block" />
          <span className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform peer-checked:translate-x-5" />
          <span className="ml-3 text-sm text-gray-700">{active ? "Active" : "Inactive"}</span>
        </label>
      </div>
